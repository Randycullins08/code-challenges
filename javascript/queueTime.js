...
function queueTime(customers, n) {
  // Initialize an array to represent the time at each till
  let tills = Array(n).fill(0);

  // Process each customer
  for (let time of customers) {
    // Find the till with the minimum current time
    let minTillIndex = tills.indexOf(Math.min(...tills));
    // Assign the customer's time to this till
    tills[minTillIndex] += time;
  }

  // The total time is the maximum time in the tills array
