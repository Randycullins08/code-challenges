...
function countSmileys(arr) {
  // Define the regular expression for a valid smiley
  const smileyRegex = /[:;][-~]?[)D]/;

  // Filter the array to include only elements that match the smiley pattern
  const smileys = arr.filter((face) => smileyRegex.test(face));
